...
class Shortener(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if "longuri" not in params.keys() or "shortname" not in params.keys():
            self.send_response(400)
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write(
                "Please provide both a long URI and a short name.".encode()
            )
            return
        ...
        print(longuri, shortname, CheckURI(longuri))
        ...
        if CheckURI(longuri):
            ...
            self.end_headers()
            self.wfile.write(
                "The URI '{}' appears to be invalid.".format(longuri).encode()
            )
